...
function initializeApp() {
  ...
  try { initializePcHealthCheck(); } catch (e) { console.error('initializePcHealthCheck failed', e); showDebugBanner('PC health init error: ' + e.message, 'error'); }
...
function initializePcHealthCheck() {
  const startup = document.getElementById('health-startup');
  const temperature = document.getElementById('health-temperature');
  const storage = document.getElementById('health-storage');
  const maintenance = document.getElementById('health-maintenance');
  const noise = document.getElementById('health-noise');
  const freeze = document.getElementById('health-freeze');
  const popups = document.getElementById('health-popups');
  const resetBtn = document.getElementById('health-reset-btn');

  const startupValue = document.getElementById('health-startup-value');
  const temperatureValue = document.getElementById('health-temperature-value');
  const storageValue = document.getElementById('health-storage-value');
  const maintenanceValue = document.getElementById('health-maintenance-value');
  ...
  if (!startup || !temperature || !storage || !maintenance || !scoreEl || !levelEl || !summaryEl || !recommendationsEl) {
    return;
  }
  ...
  function setRangeLabels() {
    startupValue.textContent = startup.value + 's';
    temperatureValue.textContent = temperature.value + 'C';
    storageValue.textContent = storage.value + '%';
    maintenanceValue.textContent = maintenance.value;
  }
  ...
  function calculateStatus() {
    const startupSeconds = Number(startup.value);
    const avgTemp = Number(temperature.value);
    const freeSpace = Number(storage.value);
    const monthsWithoutService = Number(maintenance.value);

    let score = 100;

    if (startupSeconds > 45) score -= Math.min(35, Math.round((startupSeconds - 45) * 0.22));
    if (avgTemp > 70) score -= Math.min(25, Math.round((avgTemp - 70) * 1.1));
    if (freeSpace < 25) score -= Math.min(20, Math.round((25 - freeSpace) * 0.8));
    if (monthsWithoutService > 6) score -= Math.min(15, Math.round((monthsWithoutService - 6) * 1.1));

    if (noise && noise.checked) score -= 10;
    if (freeze && freeze.checked) score -= 15;
    if (popups && popups.checked) score -= 12;

    score = clamp(score, 0, 100);

    let levelClass = 'level-warning';
    let levelText = 'Estado: Atencion';
    let summaryText = 'Tu PC todavia responde, pero ya muestra sintomas de desgaste. Un mantenimiento preventivo a tiempo evita problemas mas costosos.';
    const recommendations = [];

    if (score >= 80) {
      levelClass = 'level-good';
      levelText = 'Estado: Excelente';
      summaryText = 'Tu PC esta en buen estado. Con pequenos ajustes periodicos puedes mantener este rendimiento por mas tiempo.';
    } else if (score < 60) {
      levelClass = 'level-critical';
      levelText = 'Estado: Critico';
      summaryText = 'Tu PC necesita atencion prioritaria. Es recomendable mantenimiento tecnico para evitar fallas y perdida de datos.';
    }

    if (startupSeconds > 70) recommendations.push('Reducir apps de inicio para bajar el tiempo de arranque.');
    if (avgTemp > 75 || (noise && noise.checked)) recommendations.push('Limpiar ventilacion interna y revisar pasta termica.');
    if (freeSpace < 20) recommendations.push('Liberar espacio de disco o migrar a SSD con mayor capacidad.');
    if (monthsWithoutService >= 8) recommendations.push('Agendar mantenimiento preventivo completo este mes.');
    if (popups && popups.checked) recommendations.push('Ejecutar limpieza de malware y reforzar seguridad del navegador.');
    if (freeze && freeze.checked) recommendations.push('Diagnosticar RAM, disco y fuente para prevenir apagados o bloqueos.');

    if (!recommendations.length) {
      recommendations.push('Mantener rutina mensual de limpieza digital y actualizaciones.');
      recommendations.push('Programar respaldo automatico semanal de informacion clave.');
    }

    scoreEl.textContent = String(score);
    levelEl.textContent = levelText;
    levelEl.className = 'health-badge ' + levelClass;
    summaryEl.textContent = summaryText;
    renderRecommendations(recommendations);
  }

  [startup, temperature, storage, maintenance].forEach((input) => {
  ...
  [noise, freeze, popups].forEach((input) => {
    if (input) input.addEventListener('change', calculateStatus);
  });
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      ...
      storage.value = '28';
      maintenance.value = '7';
      if (noise) noise.checked = false;
      if (freeze) freeze.checked = false;
      if (popups) popups.checked = false;
      setRangeLabels();
      calculateStatus();
    });
  }
...
function calcularPrecios(servicio, urgency, warranty) {
  ...
  const preciosBase = {
    ...
    'ensamblaje-personalizado': 3000,
    mantenimiento: 1000,
    'limpieza-profunda': 300,
    'configuracion-red': 300,
    'configuracion-red-avanzada': 500,
    'instalacion-software': 150,
    'instalacion-software-personalizado': 300,
    'recuperacion-datos': 800,
    'recuperacion-datos-avanzada': 1500,
    asesoramiento: 200,
    'soporte-remoto': 400,
    'diagnostico-completo': 100,
    'diagnostico-hardware': 200,
    'optimizacion-sistema': 350,
    'backup-datos': 200,
    'configuracion-backup': 400,
    ...
    'instalacion-so': 500,
    'instalacion-so-personalizado': 800,
...
function getCalculatorQuoteData() {
  ...
  const serviceNames = {
    ...
    'upgrade-motherboard': 'Upgrade de Motherboard',
    'upgrade-ssd': 'Upgrade de SSD',
    'upgrade-cpu': 'Upgrade de CPU',
    'upgrade-completo': 'Upgrade Completo',
    'ensamblaje-basico': 'Ensamblaje Básico',
    'ensamblaje-gaming': 'Ensamblaje Gaming',
    'ensamblaje-personalizado': 'Ensamblaje Personalizado',
    ...
    'instalacion-software-personalizado': 'Instalación de Software Personalizado',
    'recuperacion-datos': 'Recuperación de Datos',
    'recuperacion-datos-avanzada': 'Recuperación de Datos Avanzada',
    'configuracion-red': 'Configuración de Red',
    'configuracion-red-avanzada': 'Configuración de Red Avanzada',
    'limpieza-profunda': 'Limpieza Profunda',
    'diagnostico-completo': 'Diagnóstico Completo',
    'diagnostico-hardware': 'Diagnóstico de Hardware',
    'optimizacion-sistema': 'Optimización de Sistema',
    'backup-datos': 'Backup de Datos',
    'configuracion-backup': 'Configuración de Backup',
    ...
    'sistema-enfriamiento': 'Sistema de Enfriamiento'
  };
